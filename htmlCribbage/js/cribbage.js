...
//network definitions
const localAddress = Addresses.localAddress
...
socket.on('connect_error',function(error){
	...
	if(!trylocal){ //prevent loops
		...
		var internalAddress = 'http://'+localAddress+'/';
...
socket.on("cards",function(data){
	...
	if(uncrib.tossButton==undefined){
		uncrib.tossButton=new Button(ctx.canvas.width/10,ctx.canvas.height/2,ctx.canvas.height/5,ctx.canvas.height/20,'Throw '+(uncrib.myCards.length-4)+' cards to the crib',18)
		uncrib.tossButton.click=function(){
			if(selected.length==uncrib.myCards.length-4){
				let toSend=[]
				for(i in selected){
				...
				selected=[]
			}else console.log ("you should toss "+(uncrib.myCards.length-4)+' cards')
		}
	}
});
